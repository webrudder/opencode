...
type Tenant = Parameters<typeof CloudPostgresService.create>[0]["tenant"]
type Tools = Parameters<typeof CloudPostgresService.create>[0]["tools"]
...
export function create(input: {
  ...
  const service = CloudPostgresService.create({
    ...
    modelSecretStore: input.modelSecretStore,
  })
  const queueClient = input.queueClient ?? CloudPostgresQueue.client({ client: input.client, now })
  const routes = CloudRoutes.create({
    service: {
      ...
      cancelJob: async (request) => {
        const job = await service.cancelJob(request)
        await CloudQueueRunner.run({
          client: queueClient,
          operations: [
...
export * as CloudPostgresServer from "./postgres-server"
