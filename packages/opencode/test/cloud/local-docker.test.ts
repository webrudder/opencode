...
describe("CloudLocalDocker", () => {
  ...
  test("renders local Docker deployment files and commands", () => {
    ...
    expect(result["Dockerfile.cloud-worker"]).toContain("/usr/local/bin/opencode")
    expect(result["Dockerfile.cloud-worker"]).toContain("apk add --no-cache git ripgrep python3")
  ...
  test("renders an experimental production-install image profile without changing defaults", () => {
    ...
    expect(result["Dockerfile.cloud-worker"]).toContain("RUN bun build ./packages/opencode/src/cloud/opencode-run.ts --target=bun --conditions=browser --outdir=/app/dist")
    expect(result["Dockerfile.cloud-worker"]).toContain("COPY --from=source --chown=bun:bun /app/packages/opencode/migration /migration")
